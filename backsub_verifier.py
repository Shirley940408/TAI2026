...
def _pgd_falsify(model, x, eps, true_label, restarts=5, steps=50):
    ...
    for restart in range(restarts):
        ...
        for _ in range(steps):
            ...
            loss = other.max() - logits[true_label]
...
class _Verifier:
    ...
    def run(self, deadline):
        ...
        for index, target_pos in enumerate(still_open):
            ...
            if now >= deadline:
                return False
            share = (deadline - now) / (len(still_open) - index)
            ok = self.branch_and_bound(
                target_pos, per_target_alphas[target_pos],
                per_target_bound[target_pos], now + share,
            )
            if not ok:
                return False
        return True
...
def analyze(model, x, eps, true_label):
    ...
    if _pgd_falsify(model, x, eps, true_label):
        return False
